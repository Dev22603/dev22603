...
type BaseProject =
	| (typeof portfolioData.projects.personal)[number]
	| (typeof portfolioData.projects.client)[number];

// Optional fields not present in current portfolio.json but supported for future entries.
type Project = BaseProject & {
	...
	challenges?: { problem?: string; approach?: string; outcome?: string }[];
};
...
export default function ProjectModal({
	...
	const links: Record<string, string | null | undefined> = project.links ?? {};
	const hasGithubLink = links.github;
	const hasLiveLink = links.live;
	const hasNpmLink = links.npm;
	const hasKaggleLink = links.kaggle;
	const hasPypiLink = links.pypi;
								...
								<div className="mb-6 pb-6 border-b border-[var(--color-border-primary)]">
									...
									<div className="space-y-4">
										{project.highlights.map((highlight, index) => (
											<div key={index}>
												<h4 className="text-[15px] font-semibold text-[var(--color-text-primary)] mb-1.5">
